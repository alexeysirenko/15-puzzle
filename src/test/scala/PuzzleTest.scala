...
class PuzzleTest extends FunSuite with Matchers with BoardHelpers {
  ...
  test("Update board correctly") {
    ...
    Puzzle.update(Puzzle.moveRight)(board).matrix shouldBe Vector(Vector(1, Board.EMPTY_DICE), Vector(2, 3))
    Puzzle.update(Puzzle.moveDown)(board).matrix shouldBe Vector(Vector(2, 1), Vector(Board.EMPTY_DICE, 3))
